...
export function defineRoutes(controllers:any,application:Express){
    for(let i=0;i<controllers.length;i++){
        ...
        const routeHandler:RouteHandler=Reflect.getMetadata('routeHandlers',controller)
        const controllerPath: String =Reflect.getMetadata('baseRoute',controller.constructor);
        const methods=Array.from(routeHandler.keys())

        for(let j=0;j<methods.length;j++){
            const method=methods[j]
            const routes=routeHandler.get(method)
